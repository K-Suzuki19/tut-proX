...
# 単語辞書登録用にdata.npyとstyle_data.npyを結合
a_data = []
...
#AttSeq2Seq クラスへ，保存した学習済みモデルと DataConverter クラスをロード
#GPUのセット
from chainer import cuda 
...
#ロードしたAttSeq2Seq モデルと，DataConverter クラスを使って，対話文を生成
...
#転移先のデータ(style データ) をDataConverter に入れる
...
#事前学習と同じコードを使って，データだけstyle ファイルにして，転移学習を行う
...
#転移学習のモデルと，DataConverter クラスを使って，対話文生成
def predict(model, query):
  enc_query = data_converter.sentence2ids(query, train=False)
  dec_response = model(enc_words=enc_query, train=False)
  response = data_converter.ids2words(dec_response)
